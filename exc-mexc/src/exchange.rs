...
impl MexcExchange {
    /// Create a new `MexcExchange`.
    pub fn new(public: Mexc, private: Mexc) -> Self {
        Self { public, private }
    }
}

impl Service<MakeInstrumentsOptions> for MexcExchange {
...
impl Service<MakeTickersOptions> for MexcExchange {
    ...
    fn call(&mut self, req: MakeTickersOptions) -> Self::Future {
        if req.is_prefer_trade_bid_ask() {
            ...
            ready(Ok(
                ExcServiceExt::<crate::MexcRequest>::apply(svc, &layer).boxed_clone()
            ))
        } else {
...
impl Service<MakePlaceOrderOptions> for MexcExchange {
...
impl Service<MakeCancelOrderOptions> for MexcExchange {
...
impl Service<MakeCheckOrderOptions> for MexcExchange {
...
impl Service<MakeSubscribeOrdersOptions> for MexcExchange {
...
impl Service<MakeFetchCandlesOptions> for MexcExchange {
...
impl MakeExchange for MexcExchange {
    fn name(&self) -> &str {
        "mexc"
    }
}
