...
async fn main() -> anyhow::Result<()> {
    ...
    let info = weex.get_funding_rate_history(&symbol, 2).await.unwrap();
    tracing::info!("{:?}", info);
    let info = weex.get_funding_rate(&symbol).await.unwrap();
    tracing::info!("{:?}", info);
    let info = weex.get_index_price(&symbol).await.unwrap();
    tracing::info!("{:?}", info);
    Ok(())
}
