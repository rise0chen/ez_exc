...
pub fn req_to_http<Req: Rest>(req: &Req, key: &Key) -> Result<Request, anyhow::Error> {
    ...
    if req.need_sign() {
        ...
        header.insert("X-BAPI-API-KEY", key.api_key.as_str().try_into()?);
        header.insert("X-BAPI-SIGN", signature.signature.try_into()?);
        header.insert("X-BAPI-TIMESTAMP", signature.signing.timestamp.into());
    }
