...
impl Standx {
    ...
    pub async fn perfect_symbol(&mut self, symbol: &mut Symbol) -> Result<(), ExchangeError> {
        ...
        if symbol.is_spot() {
            ...
            multi_size = 1.0;
            precision_price = info.price_tick_decimals;
            precision_size = info.qty_tick_decimals;
            min_size = info.min_order_qty;
            fee = info.taker_fee;
        }
        if symbol.multi_price != multi_price {
            tracing::error!("standx multi_price from {} to {}", symbol.multi_price, multi_price);
            symbol.multi_price = multi_price;
        }
        if symbol.multi_size.max(multi_size) / symbol.multi_size.min(multi_size) > 8.0 {
            tracing::error!("standx multi_size from {} to {}", symbol.multi_size, multi_size);
            symbol.multi_size = multi_size;
        }
        if symbol.precision != precision_size {
            tracing::warn!("standx precision_size from {} to {}", symbol.precision, precision_size);
            symbol.precision = precision_size;
        }
        if symbol.precision_price != precision_price {
            tracing::warn!("standx precision_price from {} to {}", symbol.precision_price, precision_price);
            symbol.precision_price = precision_price;
        }
        if symbol.min_size != min_size {
            tracing::warn!("standx min_size from {} to {}", symbol.min_size, min_size);
            symbol.min_size = min_size;
        }
        if symbol.min_usd != min_usd {
            tracing::warn!("standx min_usd from {} to {}", symbol.min_usd, min_usd);
            symbol.min_usd = min_usd;
        }
        if symbol.fee != fee {
            tracing::warn!("standx fee from {} to {}", symbol.fee, fee);
            symbol.fee = fee;
        }
    ...
    pub async fn get_funding_rate(&mut self, symbol: &Symbol) -> Result<FundingRate, ExchangeError> {
        ...
        Ok(FundingRate {
            ...
            time: (resp.next_funding_time.unix_timestamp_nanos() / 1_000_000) as u64,
    ...
    pub async fn get_funding_rate_history(&mut self, symbol: &Symbol, day: u8) -> Result<Vec<FundingRate>, ExchangeError> {
        ...
        resp.reverse();
        let interval = (resp[0].time - resp[1].time).whole_milliseconds() as u64;
        Ok(resp
            .into_iter()
            .map(|x| FundingRate {
                rate: x.funding_rate,
                time: (x.time.unix_timestamp_nanos() / 1_000_000) as u64,
                interval,
