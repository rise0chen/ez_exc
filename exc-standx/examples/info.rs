...
async fn main() -> anyhow::Result<()> {
    ...
    let info = standx.get_funding_rate_history(&symbol, 2).await.unwrap();
    assert!(info[0].time > info[1].time + 58 * 60 * 1000);
    tracing::info!("{:?}", info);
    let rate = standx.get_funding_rate(&symbol).await.unwrap();
    assert!(rate.time > info[0].time + 58 * 60 * 1000);
    assert!(rate.time < info[0].time + 25 * 60 * 60 * 1000);
    tracing::info!("{:?}", rate);
    let info = standx.get_index_price(&symbol).await.unwrap();
    tracing::info!("{:?}", info);
    Ok(())
}
