...
/// The APIKey definition of Bitmex.
        ...
        let raw = match format {
            ParamsFormat::Common => {
                if matches!(self.params.method(), Method::GET) {
                    ...
                    if body.is_empty() {
                        format!("{}{}{}", self.params.method(), self.params.path(), self.timestamp)
                    } else {
                        format!("{}{}?{}{}", self.params.method(), self.params.path(), body, self.timestamp)
                    }
                } else {
                    let body = serde_json::to_string(&self.params)?;
                    format!("{}{}{}{}", self.params.method(), self.params.path(), self.timestamp, body)
                }
            }
            _ => {
                unreachable!()
