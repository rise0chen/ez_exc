...
async fn main() -> anyhow::Result<()> {
    ...
    let mut order_req = PlaceOrderRequest::new(Decimal::new(1, 0), Decimal::new(4000, 0), OrderType::Limit);
    order_req.set_leverage(20.0);
    let order_id = bitmart.place_order(&symbol, order_req).await.unwrap();
    tokio::time::sleep(Duration::from_secs(32)).await;
    let order_id = bitmart.cancel_order(order_id).await.unwrap();
    let order = bitmart.get_order(order_id).await.unwrap();
    tracing::info!("{:?}", order);
    Ok(())
}
