use super::Grvt;
use exc_util::error::ExchangeError;
use exc_util::symbol::Symbol;
use exc_util::types::account::{Balance, Position};
use grvt_rust_sdk::types::{AccountSummaryRequest, PosotionsRequest};

impl Grvt {
    pub async fn get_balance(&mut self) -> Result<Balance, ExchangeError> {
        let resp = self
            .http
            .account_summary_full(&AccountSummaryRequest {
                sub_account_id: self.key.account_id.to_string(),
            })
            .await
            .map_err(|e| ExchangeError::Other(e.into()))?;
        Ok(Balance::new(0.0, resp.result.total_equity, 0.0))
    }
    pub async fn get_positions(&mut self, symbol: &Symbol) -> Result<(Position, Position), ExchangeError> {
        let symbol_id = crate::symnol::symbol_id(symbol);
        let req = PosotionsRequest {
            sub_account_id: self.key.account_id.to_string(),
            base: Some(vec![symbol.base.to_string()]),
            quote: Some(vec![symbol.quote.to_string()]),
        };
        let resp = self.http.positions_full(&req).await.map_err(|e| ExchangeError::Other(e.into()))?;
        let resp = resp.result.iter().filter(|x| x.instrument == symbol_id);

        let (mut short_size, mut short_val) = (0.0, 0.0);
        let (mut long_size, mut long_val) = (0.0, 0.0);
        for x in resp {
            let size = x.size;
            let price = x.entry_price;
            if x.size < 0.0 {
                short_size -= size;
                short_val -= size * price;
            } else {
                long_size = size;
                long_val += size * price;
            }
        }
        Ok((
            Position {
                id: String::new(),
                size: symbol.token_size(long_size),
                price: if long_size == 0.0 {
                    0.0
                } else {
                    symbol.token_price(long_val / long_size)
                },
            },
            Position {
                id: String::new(),
                size: symbol.token_size(short_size),
                price: if short_size == 0.0 {
                    0.0
                } else {
                    symbol.token_price(short_val / short_size)
                },
            },
        ))
    }
    pub async fn get_position(&mut self, symbol: &Symbol) -> Result<Position, ExchangeError> {
        self.get_positions(symbol).await.map(|(long, short)| {
            let size = long.size - short.size;
            let price = if (long.size + short.size) == 0.0 {
                0.0
            } else {
                (long.size * long.price + short.size * short.price) / (long.size + short.size)
            };
            Position {
                id: String::new(),
                size,
                price,
            }
        })
    }
}
