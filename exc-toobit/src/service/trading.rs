...
impl Toobit {
    async fn get_order_size(&mut self, symbol: &Symbol, size: f64, price: f64) -> (f64, bool) {
        ...
        if size.is_sign_positive() {
            ...
            if want_size <= short.size {
                ...
                if short.size >= min_once { (short.size, true) } else { (size, false) }
            }
        } else {
            let want_size = size.abs();
            if want_size <= long.size {
                ...
                if long.size >= min_once { (-long.size, true) } else { (size, false) }
            }
        }
    }
    ...
    pub async fn get_order(&mut self, order_id: OrderId) -> Result<Order, ExchangeError> {
        ...
        let order = if symbol.is_spot() {
            ...
            let deal_vol = symbol.token_size(resp.executed_qty);
            let deal_price = symbol.token_price(resp.avg_price.unwrap_or_default());
            Order {
                order_id: resp.order_id.to_string(),
                vol: symbol.token_size(resp.orig_qty),
                deal_vol,
                deal_avg_price: deal_price,
                fee: Fee::Quote(symbol.fee * deal_vol * deal_price),
