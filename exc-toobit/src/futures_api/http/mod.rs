...
const HOST: &str = "https://api.toobit.com";
...
pub fn req_to_http<Req: Rest>(req: &Req, key: &Key) -> Result<Request, anyhow::Error> {
    ...
    let body = if req.need_sign() {
        let signature = key.sign(req, ParamsFormat::Urlencoded, ApiKind::Common)?;
        header.insert("X-BB-APIKEY", key.api_key.as_str().try_into()?);
