...
const HOST: &str = "wss://api.gateio.ws/ws/v4/";
...
#[serde(tag = "channel", content = "result")]
pub enum RxResponse {
    ...
    pub async fn run(&self) -> Result<(), anyhow::Error> {
        ...
        for s in &self.symbols {
            let req_price = TxRequest {
                time: (OffsetDateTime::now_utc().unix_timestamp_nanos() / 1_000_000_000) as u64,
                channel: "spot.order_book",
                event: "subscribe",
                payload: vec![s.into(), "10".into(), "1000ms".into()],
            };
