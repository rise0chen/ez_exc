        ...
        let signature = match kind {
            ApiKind::SpotApi => {
                ...
                let raw = if matches!(self.params.method(), Method::GET | Method::DELETE) {
                    if body.is_empty() {
                        format!("{}#{}#{}", header, self.params.method().as_str(), self.params.path())
                    } else {
                        format!("{}#{}#{}#{}", header, self.params.method().as_str(), self.params.path(), body)
                    }
                } else {
                    format!("{}#{}#{}#{}", header, self.params.method().as_str(), self.params.path(), body)
                };
            ...
            ApiKind::FuturesApi => {
                let header = format!("validate-appkey={}&validate-timestamp={}", key.api_key, self.timestamp);
                let raw = if matches!(self.params.method(), Method::GET | Method::DELETE) {
                    if body.is_empty() {
                        format!("{}#{}", header, self.params.path())
                    } else {
                        format!("{}#{}#{}", header, self.params.path(), body)
                    }
                } else {
                    format!("{}#{}#{}", header, self.params.path(), body)
                };
